...
namespace Nop.Plugin.Misc.MailChimp.Services
{
    public class SubscriptionEventConsumer : IConsumer<EmailSubscribedEvent>, IConsumer<EmailUnsubscribedEvent>
    {
        ...
        public  void HandleEvent(EmailSubscribedEvent eventMessage)
        {
            ...
            _service.Insert(new MailChimpEventQueueRecord { Email = eventMessage.Email, IsSubscribe = true, CreatedOnUtc = DateTime.UtcNow, AdditionalInfo = eventMessage.AdditionalInfo, SuscriptionTypeId = Convert.ToInt32(eventMessage.SuscriptionType) });
        }
        ...
        public void HandleEvent(EmailUnsubscribedEvent eventMessage)
        {
            ...
            _service.Insert(new MailChimpEventQueueRecord { Email = eventMessage.Email, IsSubscribe = false, CreatedOnUtc = DateTime.UtcNow, SuscriptionTypeId = Convert.ToInt32(eventMessage.SuscriptionType) });
        }
    }
}